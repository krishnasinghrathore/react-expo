...
let time;
const NextState = /* @__PURE__ */ new WeakMap(), NextMedia = /* @__PURE__ */ new WeakMap();
  ...
  const component = import_react.default.forwardRef((propsIn, forwardedRef) => {
    ...
    if ("themeInverse" in props && (themeStateProps.inverse = props.themeInverse), "theme" in props && (themeStateProps.name = props.theme), typeof stateRef.current.isListeningToTheme == "boolean" && (themeStateProps.needsUpdate = () => !!stateRef.current.isListeningToTheme), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile") {
      ...
      if (console.info(
        ...
        childLog.length && (childLog = `(children: ${childLog})`), console.groupCollapsed(`${childLog} [inspect props, state, context \u{1F447}]`), (0, import_log.log)("props in:", propsIn), (0, import_log.log)("final props:", props), (0, import_log.log)({ state, staticConfig, elementType, themeStateProps }), (0, import_log.log)({
    ...
    if (splitStyles && (hasAnimationProp || groupName) && animationDriver?.avoidReRenders) {
      const ogSetStateShallow = setStateShallow;
      stateRef.current.updateStyleListener = () => {
        const updatedState = NextState.get(stateRef) || state, mediaState2 = NextMedia.get(stateRef), {
          group,
          hasDynGroupChildren,
          unmounted,
          animation,
        ...
        groupContext && notifyGroupSubscribers(
          groupContext,
        ...
        NextMedia.set(stateRef, next), stateRef.current.updateStyleListener?.();
      }, stateRef.current.setStateShallow = (nextOrGetNext) => {
        const prev = NextState.get(stateRef) || state, next = typeof nextOrGetNext == "function" ? nextOrGetNext(prev) : nextOrGetNext;
        ...
        if (canAvoidReRender) {
          ...
          NextState.set(stateRef, updatedState), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile" && (console.groupCollapsed("[\u26A1\uFE0F] avoid setState", next, { updatedState, props }), console.info(stateRef.current.host), console.groupEnd()), stateRef.current.updateStyleListener?.();
        } else
          process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile" && console.info("[\u{1F40C}] re-render", { canAvoidReRender, next }), ogSetStateShallow(next);
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`
    ), process.env.NODE_ENV === "development" && time && time`destructure`, splitStyles && groupContext && // avoids onLayout if we don't need it
...
` && console.error(
    ...
    process.env.NODE_ENV === "development" && time && time`spaced-as-child`, splitStyles || (content = propsIn.children, elementType = BaseViewComponent, viewProps = {
      style: {
        display: "contents"
      }
    });
