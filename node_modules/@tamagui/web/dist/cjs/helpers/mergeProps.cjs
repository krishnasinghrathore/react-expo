...
const mergeProps = (a, b, inverseShorthands) => {
  ...
  if (b && Object.keys(b).length > 0 && Object.keys(b).some(key => (key in import_pseudoDescriptors.pseudoDescriptors || key === "variant") && key in a && key in out)) {
    const reordered = {};
    for (const key in b) (key in import_pseudoDescriptors.pseudoDescriptors || key === "variant") && key in out && (reordered[key] = out[key]);
