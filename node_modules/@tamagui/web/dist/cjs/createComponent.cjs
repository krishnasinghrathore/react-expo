...
const NextState = /* @__PURE__ */new WeakMap(),
  NextMedia = /* @__PURE__ */new WeakMap();
      ...
      stateRef.current.updateStyleListener = () => {
        ...
        if (canAvoidReRender) {
          ...
          NextState.set(stateRef, updatedState), process.env.NODE_ENV === "development" && debugProp && debugProp !== "profile" && (console.groupCollapsed("[\u26A1\uFE0F] avoid setState", next, {
            updatedState,
            props
          }), console.info(stateRef.current.host), console.groupEnd()), stateRef.current.updateStyleListener?.();
...
If you meant to do this, you can disable this warning - either change untilMeasured and group at the same time, or do group={conditional ? 'name' : undefined}`), process.env.NODE_ENV === "development" && time && time`destructure`, splitStyles && groupContext &&
    // avoids onLayout if we don't need it
...
` && console.error(`Unexpected text node: ${item}. A text node cannot be a child of a <${staticConfig.componentName || propsIn.tag || "View"}>.`, props);
    ...
    if (process.env.NODE_ENV === "development" && time && time`context-override`, startedUnhydrated) {
      const styleTags = (0, import_react.useMemo)(() => {
        if (splitStyles) return (0, import_wrapStyleTags.getStyleTags)(Object.values(splitStyles.rulesToInsert));
      }, []);
