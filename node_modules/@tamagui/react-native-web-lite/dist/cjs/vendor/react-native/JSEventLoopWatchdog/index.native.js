...
var import_infoLog = __toESM(require("../infoLog")), JSEventLoopWatchdog = {
  getStats: function() {
    return {
      stallCount,
      totalStallTime,
  ...
  reset: function() {
    (0, import_infoLog.default)("JSEventLoopWatchdog: reset"), totalStallTime = 0, stallCount = 0, longestStall = 0, lastInterval = window.performance.now();
  },
  addHandler: function(handler) {
    handlers.push(handler);
  },
  install: function(param) {
    var { thresholdMS } = param;
    if (acceptableBusyTime = thresholdMS, installed)
      return;
    installed = !0, lastInterval = window.performance.now();
    function iteration() {
      var now = window.performance.now(), busyTime = now - lastInterval;
...
}, acceptableBusyTime = 0, installed = !1, totalStallTime = 0, stallCount = 0, longestStall = 0, lastInterval = 0, handlers = [], JSEventLoopWatchdog_default = JSEventLoopWatchdog;
