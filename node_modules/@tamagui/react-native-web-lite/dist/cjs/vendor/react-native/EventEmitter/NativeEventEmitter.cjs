...
class NativeEventEmitter {
  ...
  addListener(eventType, listener, context) {
    ...
    let subscription = import_RCTDeviceEventEmitter.default.addListener(eventType, listener, context);
    return {
      remove: () => {
  ...
  removeListener(eventType, listener) {
    this._nativeModule?.removeListeners(1), import_RCTDeviceEventEmitter.default.removeListener(eventType, listener);
  }
  emit(eventType, ...args) {
    import_RCTDeviceEventEmitter.default.emit(eventType, ...args);
  }
  removeAllListeners(eventType) {
    (0, import_react_native_web_internals.invariant)(eventType != null, "`NativeEventEmitter.removeAllListener()` requires a non-null argument."), this._nativeModule?.removeListeners(this.listenerCount(eventType)), import_RCTDeviceEventEmitter.default.removeAllListeners(eventType);
  }
