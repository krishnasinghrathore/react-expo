...
import createCSSStyleSheet from "./createCSSStyleSheet.native.js";
import createOrderedCSSStyleSheet from "./createOrderedCSSStyleSheet.native.js";
...
function createSheet(root) {
  ...
  function createSheet2() {
    if (!sheet) if (canUseDOM) {
      ...
      if (sheets.length === 0) sheet = createOrderedCSSStyleSheet(createCSSStyleSheet(id)), initialRules.forEach(function (rule) {
        sheet.insert(rule, 0);
    ...
    } else sheets.length === 0 ? (sheet = createOrderedCSSStyleSheet(createCSSStyleSheet(id)), initialRules.forEach(function (rule) {
      sheet.insert(rule, 0);
