...
const DialogContext = createStyledContext(
  ...
  (props, forwardRef) => {
    const { scope, forceMount, children, ...frameProps } = props, dialogRef = React.useRef(null), ref = composeRefs(dialogRef, forwardRef), context = useDialogContext(scope), isMountedOrOpen = forceMount || context.open, [isFullyHidden, setIsFullyHidden] = React.useState(!isMountedOrOpen), isAdapted = useAdaptIsActive(context.adaptScope), isVisible = !isFullyHidden;
    isWeb && useIsomorphicLayoutEffect(() => {
      const node = dialogRef.current;
      node instanceof HTMLDialogElement && (isVisible ? node.show() : node.close());
    }, [isVisible]), isMountedOrOpen && isFullyHidden && setIsFullyHidden(!1);
    const handleExitComplete = React.useCallback(() => {
      setIsFullyHidden(!0);
    }, []), zIndex = getExpandedShorthand("zIndex", props), contents = /* @__PURE__ */ jsx(StackZIndexContext, { zIndex: resolveViewZIndex(zIndex), children: /* @__PURE__ */ jsx(AnimatePresence, { passThrough: isAdapted, onExitComplete: handleExitComplete, children: isMountedOrOpen || isAdapted ? children : null }) });
    if (isFullyHidden && !isAdapted)
      return null;
    const framedContents = /* @__PURE__ */ jsx(
      DialogPortalFrame,
      {
        ref,
