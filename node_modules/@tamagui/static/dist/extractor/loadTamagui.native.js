...
async function loadTamagui(propsIn) {
  ...
  try {
    ...
    if (!bundleInfo)
      return console.warn("No bundled config generated, maybe an error in bundling. Set DEBUG=tamagui and re-run to get logs."), resolvePromise(null), null;
    await generateThemesAndLog(props);
    var maybeTamaguiConfig = bundleInfo.tamaguiConfig;
...
function loadTamaguiBuildConfigSync(tamaguiOptions) {
  ...
  if (fsExtra.existsSync(buildFilePath)) {
    ...
    try {
      ...
      if (tamaguiOptions != null && tamaguiOptions.config && out.config)
        throw new Error("You're configuring tamagui from both the plugin and tamagui.build.ts, please choose one or the other.");
      tamaguiOptions = {
        ...tamaguiOptions,
        ...out
      };
